...
//! sync cursors verbatim, a role edit changes only the role, removals delete,
//! additions become user_assigned — and an invalid desired set mutates
//! nothing.
...
#[test]
fn role_edit_updates_only_the_role() {
    ...
    // an automatic binding stays revisable: the role edit must not upgrade
    // its provenance to user_assigned
    assert_eq!(after[0].source, binding_source::AUTO);
    assert_eq!(after[0].created_at, before[0].created_at);
}
