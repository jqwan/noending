...
//! builds a whole object by hand: these are only ever the *input* to such a
//! path, so the domain stays free of fixture-only constructors.
...
pub fn ensure_session(
    ...
    let _ = id; // row id is store-assigned; callers use the returned Session
