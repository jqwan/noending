...
* **Path identity is pure lexical.** `workspace_paths.id = "path-" + sha256(identity_key(canonical_path))`, computed by `workspace::identity` with no filesystem access: no `fs::canonicalize`, no symlink resolution, no "does it exist" input. Existence and Git state are *observations* on the row, never part of the key.
* **"Same location?" comparisons use `identity::same_location`, never `==` and never `path_key` equality.** On a Windows volume case is part of the location relation but not of the display name: fold for comparison, never for display. `path_identity` folds by the same location key — the stored `workspace_paths.id` is derived from it, so Windows never had an id to preserve — and it stays **byte-for-byte unchanged on macOS/Unix**, where a change would be a data migration.
