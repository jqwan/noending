...
* **"Same location?" comparisons use `identity::same_location`, never `==` and never `path_key` equality.** On a Windows volume case is part of the location relation but not of the display name: fold for comparison, never for display. `path_identity` itself deliberately does *not* fold, because widening it would change stored ids and require a data migration.
